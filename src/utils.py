    ...
    try:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if isinstance(audio_file, str):
                # If audio_file is already a path, just use it
                temp_file_path = audio_file
                file_extension = os.path.splitext(audio_file)[1].lower()
            else:
                # If audio_file is bytes or a file object, save it to temp file
                try:
                    file_name = getattr(audio_file, 'name', 'temp_audio')
                    file_extension = os.path.splitext(file_name)[1].lower() or '.wav'
                    temp_file_path = os.path.join(temp_dir, f"input_audio{file_extension}")
                    ...
                    raise Exception(f"Kon het audiobestand niet verwerken: {str(e)}")

            if not temp_file_path or not os.path.exists(temp_file_path):
                raise Exception("Geen geldig audiobestand gevonden")
            ...
            if file_extension == '.mp4':
                try:
                    ...
                    logger.error(f"Error converting MP4: {str(e)}")
                    raise Exception(f"Kon het MP4-bestand niet converteren: {str(e)}")
            ...
            try:
                ...
                raise Exception(f"Fout bij het verwerken van audio: {str(e)}")
